...
As the size of electronic components grow ever smaller, a new category of devices has emerged, called \textit{wearables}. Two types of wearables in particular, are beginning to see wide acceptance into mainstream use. These are the fitness band and the smartwatch. In just a couple of years, they have gone from being novelty devices, ridiculed for "looking silly" and having ridiculously short battery life, to being genuinely desirable devices for a lot of people.

While the distinction between smartwatches and fitness bands can sometimes be a bit muddy -- some fitness bands are starting to adopt many smartwatch features and vice versa -- fitness bands tend to be less focused on user interface and more focused on the usefulness of their sensors. Smartwatches, on the other hand, tend to have some fitness-related features, but these are often fewer and more limited. Smartwatches do tend to have a larger screen, a more powerful GUI and usually a smartphone-like operating system.
...
Wearables are new technology, and there is not a lot of existing studies to be found on the subject. The authors have not been able to find a single existing study on the forensics of fitness bands.
...
In this study, we have focused on two devices -- a smartwatch called \textit{LG Watch Urbane} and a fitness band called \textit{Fitbit Charge HR}. Below are detailed descriptions of our findings for each of the devices.


\subsection{LG Watch Urbane}
...
\subsubsection{Getting started with \texttt{adb}}
...
\subsubsection{Debug log acquisition}
...
\subsubsection{The \texttt{WearableConn} log tag}
...
\subsubsection{The \texttt{DisplayManagerService} log tag}
...
\subsubsection{Log file statistics}
...
\subsubsection{Extracting the filesystem}
...
\subsubsection{Extracting data from the Google Fit API}
...
\item[Popularity] According to the sales representatives asked it was a fairly popular brand; the author had also heard about Fitbit prior to starting this paper, the company also claims to have sold more than 30 million devices (Fitbit 2015)
...
\subsubsection{Fitbit ecosystem}
...
\subsubsection{Extracting data from the device itself}
...
For the purpose of this article the megadump is the most interesting, as it is stored on the device and also on Fitbits own servers. Although the most recent live data is stored on the smartphone, there is no history associated with it as far as we can tell, so it has less value in our scenario. Aprville

The entire megadump consists of hexadecimal characters as shown in figure~\ref{fig:megadump}; a comparison of multiple dumps indicates that there is a 16-byte header and a 9-byte footer with some apparently static bytes. This has also been indicated by (Allard 2014a) as well as (Aprville 2015b).
...
\subsubsection{Downloading data from the Fitbit API}
...
\subsubsection{Extracting the data from a smartphone}
...
\bibliography{Forensics of Wearable Devices}
